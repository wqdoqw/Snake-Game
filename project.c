 ...
 *
 * Author: Peter Sutton. Modified by Sang Ik Park
 */ 
...
void splash_screen(void) {
	...
	move_cursor(3,3);
	printf_P(PSTR("Snake 43408135"));
	
	move_cursor(3,5);
	set_display_attribute(FG_GREEN);	// Make the text green
	// Modify the following line
	printf_P(PSTR("CSSE2010/7201 Snake Project by Sang Ik Park"));	
	...
	while(1) {
		set_scrolling_display_text("SNAKE 43408135", colour);
...
void play_game(void) {
	...
	while(1) {
		...
		if(get_clock_ticks() >= last_move_time + spd) {
